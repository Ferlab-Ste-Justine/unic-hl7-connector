 ...
 */
class Application @throws[IOException]
() extends RouteBuilder {
  final private[minio] val properties = MinioExampleUtils.loadMinioPropertiesFile

  @Produces
  @Named("minioClient") private[minio] def minioClient = {
  ...
  override def configure(): Unit = {
    from("minio://bucket-name?deleteAfterRead=false&maxMessagesPerPoll=25&delay=5000")
      ...
      .idempotentConsumer(
        header("CamelMinioETag"),
        FileIdempotentRepository
          .fileIdempotentRepository(new File("target/file.data"), 250, 512000))
